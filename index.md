...
Once this is all set, I hit "create" and Sentinel now has an alert rule. Now it will be querying my machine's event logs every five minutes to look at data from five minutes ago to see if there were any events that fit the query's criteria. If there is even one match, an alert will be generated. So now, to test this, I will be setting off the alarm myself.

# Tripping My Own Alarm

My VM having RDP open means literally any machine with RDP can remote into it, including my own. I launch my own computer's RDP and specify my virtual machine's IP address.

- _Figure 16_: Opening up my computer's Remote Desktop Connection and specifying my virtual machine's IP address. All I need to do this is my machine's public IP address. Although I knew this from the start, finding this out would not take too much digging.

<p align="center">
  <img width="404" height="248" src="assets/fig25.png">
</p>

The IP address is good. I am then prompted to enter the credentials of my virtual machine.

- _Figure 17_: The login window for my virtual machine. There are some failed attempts because I was not sure exactly which credentials it was asking for. I suppose it stands to reason they were the credentials I set earlier. As mentioned, my password is quite strong, so I probably will not be getting the breach activity I was hoping for.

<p align="center">
  <img width="404" height="248" src="assets/fig26.png">
</p>

The credentials are good, and then I get the warning window prior to connecting regarding my virtual machine not having a certificate from a trusted certifying authority. Nevertheless, I know that no one has been in here yet so I connect.

As expected, my virtual machine is rather unremarkable. It is merely a blank machine running Windows 11 Pro. Interestingly, it is connected to the Internet and has all Windows services up and running. I am not really meant to do anything here, so I close it out. Now I know that I have signed into my virtual machine with RDP and that even though it is my virtual machine, it does not recognize my personal computer. Therefore, Sentinel should have logged my successful login as an incident and generated an alert.

Returning to Azure, and opening Sentinel's dashboard, I am met with this:

- _Figure 18_: The "incidents" widget on Sentinel's dashboard. The SIEM tool recorded one incident at medium severity which leads me to believe that this must be the rule I created.

<p align="center">
  <img width="847" height="464" src="assets/fig29.png">
</p>

Sure enough, an incident was generated with a time that corresponds to my login time. For good measure, I logged into the virtual machine a few more times just to see if more alerts would be generated. As expected, there were no additional alerts because Sentinel will only generated an alert for a user's initial login.

- _Figure 19_: The information panel regarding my alert. The severity was record as medium and was generated by my instance of Sentinel. It was also generated because it was an intial sign in and the rule Sentinel followed is the one I created. These are also the times, in my own time zone, when I logged into the virtual machine.

<p align="center">
  <img width="442" height="625" src="assets/fig30.png">
</p>

Now although my SIEM is set to create alerts for the events I specified, that is still monitoring many other events that are happening to my computer. After leaving my home lab alone for a day, I returned to find almost sixty THOUSAND attempts to breach my system. Out of curiosity, I viewed these logs and found that they were events generated when there was a failed login attempt. These logs came from unrecognized usernames and public IP addresses. I looked a few of these up and found users from Russia, Netherlands, China, and Brazil. I was shocked because these are all people who found my vulnerable machine in record time and tried to brute force it. I suspect they only gave it a few tries before moving on to the next machine. Regardless, had I given my machine a weak password, my system almost certainly would have been breached by now. I feel that this was an important eye-opener to the realities of the digital world. That is, how essential it is to make sure that your machines are hardened and protected because it literally only takes a few hours for even an amateur hacker to find your machine and attempt to break into it.

# Conclusion

I therefore deem the lab a success. I have a functional (and vulnerable) virtual machine complete with its own log analytics system and a SIEM to monitor the logs and generate alerts for anything suspicious. The next part of this lab will deal with creating an additional virtual machine running Linux which will ingest threat indicators for my virtual machine. That is, I will be creating my own threat intelligence feed via an API call that will further augment my SIEM.
